...
/// <summary>
/// Splits a Markdown file's leading `---`-delimited frontmatter block from its body and parses the
/// block with YamlDotNet's representation model (YamlMappingNode preserves key order, unlike a
/// plain dictionary). Generic over which <see cref="FrontmatterMapping"/> subclass gets constructed from
/// the parsed mapping, via <paramref name="factory"/> - shared by Skills.SkillFrontmatterYaml and
/// OutputStyles.OutputStyleFrontmatterYaml, both thin per-feature wrappers around this. Kept free of
/// any file I/O so it's unit-testable against plain string literals.
/// </summary>
internal static class FrontmatterYaml
{
    ...
    /// </summary>
    public static bool TryParse<T>(string rawText, Func<YamlMappingNode, T> factory, out T? frontmatter, out string? rawFrontmatterBlock, out string body)
        where T : FrontmatterMapping
    {
        ...
        if (closingIndex < 0)
        {
            // Unterminated block - everything after the opening --- is the yaml block, body is empty.
        ...
        return TryParseBlock(yamlBlock, factory, out frontmatter);
    }

    /// <summary>
    /// Parses just the inner YAML text of a frontmatter block (no surrounding `---` markers) - used
    /// by an editor's own frontmatter textarea, which holds exactly that. True only when it parses
    /// as a YAML mapping; false for invalid YAML or a non-mapping root (a bare scalar or sequence),
    /// matching <see cref="TryParse{T}"/>'s own rules for the same two cases.
    /// </summary>
    public static bool TryParseBlock<T>(string yamlBlock, Func<YamlMappingNode, T> factory, out T? frontmatter)
        where T : FrontmatterMapping
    {
    ...
    /// preserves whatever key order and per-field scalar/sequence shape the frontmatter's setters
    /// left it in.
    /// </summary>
    public static string Serialize(FrontmatterMapping frontmatter, string body)
    {
    ...
    /// newline forced - used by an editor to refresh its frontmatter textarea after a left-panel
    /// field change, without also touching the body.
    /// </summary>
    public static string SerializeBlock(FrontmatterMapping frontmatter)
    {
    ...
    private static string EmitMapping(YamlMappingNode mapping)
    {
        ...
        // with a "...\n" document-end marker - neither belongs inside a frontmatter block, so
        // normalize line endings first and trim the end marker back off.
